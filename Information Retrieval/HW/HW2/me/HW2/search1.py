...
def run_search(dict_file, postings_file, queries_file, results_file):
    ...
    for i in range(len(queries)):
        ...
        if queries[i].count('(') - queries[i].count(')'):
            queries[i] = []
        else:
...
def Tokenize(Query):
    ...
    for i in range(len(QueryList)):
        ...
        # NOT NOT
        if current_token == 'NOT':
            if len(NewQueryList) and NewQueryList[-1] == 'NOT':
                NewQueryList.pop(-1)
            else:
        ...
        # left bracket
            ...
            while len(token) and token[0] == '(':
                NewQueryList.append('(')
                token = token[1:]
            NewQueryList.append(token)
        # right bracket
        elif current_token[-1] == ')':
            rightBracket = [')']
            token = current_token[:-1]
            while len(token) and token[-1] == ')':
                token = token[:-1]
                rightBracket.append(')')
            NewQueryList.append(token)
            NewQueryList.extend(rightBracket)
        else:
            NewQueryList.append(current_token)

    for id in range(len(NewQueryList)):
        if not (NewQueryList[id] in operators_info.keys()):
            NewQueryList[id] = ps.stem(NewQueryList[id]).lower()
    return NewQueryList
