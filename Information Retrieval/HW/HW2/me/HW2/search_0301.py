...
def run_search(dict_file, postings_file, queries_file, results_file):
    ...
    for i in range(34148):
        ...
        postings = pickle.load(postingFile)
        print("no. " + str(i + 1) + " pair, term: " + str(data) + ", postings: " + str(postings))

    queries = Tokenize(queries_file)
    queries = ShuntYard(queries)
    
    docIDs = os.listdir('nltk_data/corpora/reuters/training/')
    universe = docIDs.sort()

    query_result = []
    while len(queries):
        current_token = queries.pop(0)
        ...
        # Not a operator
            ...
            postingFile.seek(data["pointer"])
            postings = pickle.load(postingFile)
            query_result.append(postings[-1])
    
    outputToFiles(query_result)
